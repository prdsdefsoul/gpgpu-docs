...
`tools/check_terms.py` 是本项目自建的文档质量检查，在 CI 里作为门禁运行：

| 检查 | 内容 |
| --- | --- |
| 禁用译法 | 扫描全部正文，发现 `tools/terminology.yml` 里禁止的写法并给出应改用的译法与原因 |
| 中英页面对等 | 每个 `xxx.md` 必须有对应的 `xxx.en.md`，反之亦然 |
| 中英术语表对等 | 中英文术语表必须定义同一批英文术语 |

```bash
python3 tools/check_terms.py
```

**为什么需要它**：多语言文档最容易失守的不是翻译质量，而是同一个概念在不同页面出现不同译法。这类问题分散在几十个文件里、每处单看都没错，人工评审发现不了，只有机器扫得动。

**实现上的一个坑**：中文没有词边界，直接做子串匹配会误伤——禁止「线程组」时，「线程组织方式」也会被判违规。所以规则支持 `unless` 字段声明合法的更长词，匹配落在其中时不计。这个缺陷是检查器写完第一版跑出来的第一个结果。
